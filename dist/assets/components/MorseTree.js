import { decode, displaySequence, encode, learningPool, sequencePrefixes } from '../features/morse/morse-core.js';
...
export function MorseTree({ sequence, targetSequence = '', showNumbers, codeSet, language }) {
    const usePhotoPlate = codeSet === 'latin' && !showNumbers;
    return (React.createElement("div", { className: "tree-shell" },
        ...
        React.createElement("div", { className: "tree-scroll", tabIndex: 0 }, usePhotoPlate ? (React.createElement(PhotoPlate, { sequence: sequence, targetSequence: targetSequence })) : (React.createElement(CodePlate, { sequence: sequence, targetSequence: targetSequence, codeSet: codeSet, includeNumbers: showNumbers }))),
        React.createElement(CandidateStrip, { sequence: sequence, codeSet: codeSet, language: language })));
}
function PhotoPlate({ sequence, targetSequence }) {
    const active = new Set(sequencePrefixes(sequence));
    const target = new Set(sequencePrefixes(targetSequence));
    return (React.createElement("svg", { className: "morse-tree morse-plate photo-plate", viewBox: `0 0 ${WIDTH} ${HEIGHT}`, role: "img", "aria-label": "Morse code plate" },
...
function CodePlate({ sequence, targetSequence, codeSet, includeNumbers }) {
    const title = codeSet === 'wabun' ? 'WABUN' : 'MORSE';
    const items = learningPool(codeSet, includeNumbers)
        .map(char => ({ char, code: encode(char, codeSet) ?? '' }))
        .filter(item => item.code);
    return (React.createElement("div", { className: "code-plate", role: "img", "aria-label": `${title} code plate` },
        React.createElement("div", { className: "code-plate-title", "aria-hidden": "true" },
            React.createElement("span", null, title),
            React.createElement("span", null, "CODE")),
        React.createElement("div", { className: "code-plate-grid" }, items.map(item => {
            const exact = sequence === item.code;
            const inputMatch = Boolean(sequence && item.code.startsWith(sequence));
            const targetMatch = Boolean(targetSequence && item.code === targetSequence);
            return (React.createElement("div", { key: item.char, className: `code-plate-cell ${exact ? 'current' : ''} ${inputMatch ? 'prefix-match' : ''} ${targetMatch ? 'target' : ''}` },
                React.createElement("strong", null, item.char),
                React.createElement("div", { className: "code-plate-path", "aria-label": `${item.char}: ${displaySequence(item.code)}` }, [...item.code].map((symbol, index) => {
                    const isInputNode = inputMatch && index < sequence.length;
                    const isTargetNode = !isInputNode && targetMatch && index < targetSequence.length;
                    return (React.createElement("span", { key: `${item.char}-${index}`, className: `plate-mini-node ${symbol === '.' ? 'dot' : 'dash'} ${isInputNode ? 'active' : ''} ${isTargetNode ? 'target' : ''}` }));
                }))));
        }))));
}
function CandidateStrip({ sequence, codeSet, language }) {
    return (React.createElement("div", { className: "candidate-strip" },
