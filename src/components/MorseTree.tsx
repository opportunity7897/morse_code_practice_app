import { decode, displaySequence, encode, learningPool, sequencePrefixes, type CodeSet } from '../features/morse/morse-core.js';
  ...
  return (
    <div className="tree-shell">
      ...
      <div className="tree-scroll" tabIndex={0}>
        {usePhotoPlate ? (
          <PhotoPlate sequence={sequence} targetSequence={targetSequence} />
        ) : (
          <CodePlate sequence={sequence} targetSequence={targetSequence} codeSet={codeSet} includeNumbers={showNumbers} />
        )}
      </div>
      <CandidateStrip sequence={sequence} codeSet={codeSet} language={language} />
    </div>
  );
}

function PhotoPlate({ sequence, targetSequence }: { sequence: string; targetSequence: string }) {
  const active = new Set(sequencePrefixes(sequence));
  const target = new Set(sequencePrefixes(targetSequence));

  return (
    <svg className="morse-tree morse-plate photo-plate" viewBox={`0 0 ${WIDTH} ${HEIGHT}`} role="img" aria-label="Morse code plate">
...
function CodePlate({ sequence, targetSequence, codeSet, includeNumbers }: {
  sequence: string; targetSequence: string; codeSet: CodeSet; includeNumbers: boolean;
}) {
  ...
  return (
    <div className="code-plate" role="img" aria-label={`${title} code plate`}>
      <div className="code-plate-title" aria-hidden="true"><span>{title}</span><span>CODE</span></div>
      <div className="code-plate-grid">
        {items.map(item => {
          ...
          return (
            <div key={item.char} className={`code-plate-cell ${exact ? 'current' : ''} ${inputMatch ? 'prefix-match' : ''} ${targetMatch ? 'target' : ''}`}>
              <strong>{item.char}</strong>
              <div className="code-plate-path" aria-label={`${item.char}: ${displaySequence(item.code)}`}>
                {[...item.code].map((symbol, index) => {
                  const isInputNode = inputMatch && index < sequence.length;
                  const isTargetNode = !isInputNode && targetMatch && index < targetSequence.length;
                  return (
                    <span
                      key={`${item.char}-${index}`}
                      className={`plate-mini-node ${symbol === '.' ? 'dot' : 'dash'} ${isInputNode ? 'active' : ''} ${isTargetNode ? 'target' : ''}`}
                    />
                  );
                })}
              </div>
...
function CandidateStrip({ sequence, codeSet, language }: { sequence: string; codeSet: CodeSet; language: Language }) {
  return (
